            ...
            <div className='scene-header-right'>
                ...
                })}>{t('btn.save')}</Button>
                {
                    run_status === 'running'
                        ...
                        : <Button className='runBtn' disabled={run_status === 'running'} style={{ background: from === 'plan' ? '#2BA58F' : 'var(--run-green)' }} preFix={<SvgCaretRight />} onClick={() => runScene()}>{from === 'scene' ? t('btn.run') : t('btn.runScene')}</Button>
                }
            </div>
